...
exports.register = async (req, res) => {
  try {
    ...
    const user = await User.create({ name, email: email.toLowerCase(), password, phone: phone || '' })
    sendToken(user, 201, res)
  } catch (err) {
    res.status(500).json({ success: false, message: err.message || 'Database error during registration' })
  }
}
...
exports.login = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: err.message || 'Database connection error' })
  }
}
